...
const MaintenancePage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const dataToSubmit = {
            ...
            reported_by_id: requestData.reported_by_id,
            status: requestData.status || 'Pending',
        };

        let error;
        if (currentRequest) {
            const { error: updateError } = await supabase.from('maintenance_requests').update(dataToSubmit).eq('id', currentRequest.id);
            error = updateError;
        } else {
            const { error: insertError } = await supabase.from('maintenance_requests').insert([dataToSubmit]);
            error = insertError;
        }
                    ...
                    <table className="min-w-full">
                        ...
                        >
                            {loading ? (
                                ...
                                <tr><td colSpan="6" className="text-center py-10 text-base-content-secondary">No maintenance requests found.</td></tr>
                            )}
                        </motion.tbody>
                    </table>
                </div>
