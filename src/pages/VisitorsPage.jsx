...
import { Loader, LogOut, Trash2 } from 'lucide-react';
...
const VisitorsPage = () => {
    ...
    const handleAddVisitor = async (e) => {
        ...
        const { visitorName, student_id } = Object.fromEntries(formData.entries());

        const { error } = await supabase.from('visitors').insert([
            { visitor_name: visitorName, student_id, status: 'In' }
        ]);
    ...
    const handleCheckOut = async (visitorId) => {
        ...
        const { error } = await supabase
            .from('visitors')
            .update({ status: 'Out', check_out_time: new Date().toISOString() })
            .eq('id', visitorId);
                    ...
                    <table className="min-w-full">
                        ...
                        >
                            {loading ? (
                                ...
                                <tr><td colSpan="6" className="text-center py-10 text-base-content-secondary">No visitor logs found.</td></tr>
                            )}
                        </motion.tbody>
                    </table>
                </div>
